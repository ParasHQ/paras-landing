...
class Near {
	...
	async authToken() {
		...
		try {
			const accountId = this.currentUser.accountId.toString()
			const expiry = (new Date().getTime() + 3600000).toString() // one hour
			const msg = `${accountId}&${expiry}`
			const arr = new Array(msg)
			...
			const payload = [accountId, expiry, pubKey, signature]
