...
class WalletHelper {
	...
	isAuthTokenExpiryValid(authToken) {
		const expiry = Base64.decode(authToken).split('&')[1]
	...
	async authToken() {
		if (
			this.activeWallet === walletType.web &&
			(!near._authToken || !this.isAuthTokenExpiryValid(near._authToken))
		) {
		...
		if (
			this.activeWallet === walletType.sender &&
			(!senderWallet.authToken || !this.isAuthTokenExpiryValid(senderWallet.authToken))
		) {
			return await senderWallet.generateAuthToken()
		}
