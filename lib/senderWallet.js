...
class SenderWallet {
	...
	async signIn() {
		...
		if (resSignIn && resSignIn.accessKey) {
			...
			this.currentUser = {
				accountId,
				balance: (await window.near.getBalance()).response,
			}
	...
	signAndSendTransaction({ transactions, methodName, args, deposit, contractId, gas }) {
		...
		return window.near.signAndSendTransaction({
